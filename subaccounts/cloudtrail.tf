...
resource "aws_cloudwatch_log_group" "cloudtrail" {
  ...
  kms_key_id        = var.config.organization_kms_arn
...
resource "aws_s3_bucket_server_side_encryption_configuration" "cloudtrail" {
  ...
  rule {
    apply_server_side_encryption_by_default {
      kms_master_key_id = var.config.organization_kms_arn
...
resource "aws_s3_bucket" "cloudtrail_log" {
  provider = aws.log_archive
  bucket   = "${local.name_prefix}cloudtrail-log-${random_pet.cloudtrail.id}"
...
resource "aws_s3_bucket_acl" "cloudtrail_log" {
  provider = aws.log_archive
...
resource "aws_s3_bucket_logging" "cloudtrail" {
  provider      = aws.log_archive
  bucket        = aws_s3_bucket.cloudtrail.id
  target_bucket = aws_s3_bucket.cloudtrail_log.id
  target_prefix = "log/"
}
